...
public sealed partial class SharedEntityEffectsSystem
{
    ...
    /// </summary>
    public void TryApplyEffect(EntityUid target, [ForbidLiteral] ProtoId<EntityEffectPrototype> id, float scale = 1f, EntityUid? user = null, bool predicted = true) // Oathlord - add predicted
    {
        var proto = ProtoMan.Index(id);
        if (_condition.TryConditions(target, proto.Conditions))
            ApplyEffects(target, proto.Effects, scale, user, predicted: predicted); // Oathlord
    }
}
